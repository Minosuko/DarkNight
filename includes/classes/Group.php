...
class Group {
    ...
    public function create($name, $about, $privacy, $creator_id) {
        ...
        $sql = "INSERT INTO $db_post.groups (group_name, group_about, group_privacy, created_by, created_time) 
    ...
    public function delete($group_id, $user_id) {
        ...
        $check = $this->db->query("SELECT created_by FROM $db_post.groups WHERE group_id = $group_id");
        ...
        $sql = "DELETE FROM $db_post.groups WHERE group_id = $group_id";
        return $this->db->query($sql);
    }
    ...
    public function addMember($group_id, $user_id, $role = 0, $status = 1) {
        ...
        $sql = "INSERT INTO $db_post.group_members (group_id, user_id, role, status, joined_time) 
    ...
    public function getInfo($group_id, $current_user_id = 0) {
        ...
        $sql = "SELECT g.*, 
                ...
                LEFT JOIN $db_post.group_members m ON m.group_id = g.group_id AND m.user_id = $current_user_id
    ...
    public function isMember($group_id, $user_id) {
        ...
        $group_id = (int)$group_id;
        $user_id = (int)$user_id;
        $sql = "SELECT id FROM $db_post.group_members WHERE group_id = $group_id AND user_id = $user_id AND status = 1";
        $query = $this->db->query($sql);
    ...
    public function removeMember($group_id, $user_id) {
        ...
        $group_id = (int)$group_id;
        $user_id = (int)$user_id;
        $sql = "DELETE FROM $db_post.group_members WHERE group_id = $group_id AND user_id = $user_id";
        return $this->db->query($sql);
    }
    ...
    public function updateMemberRole($group_id, $user_id, $role) {
        ...
        $sql = "UPDATE $db_post.group_members SET role = $role WHERE group_id = $group_id AND user_id = $user_id";
        return $this->db->query($sql);
    }
    ...
    public function getPosts($group_id, $limit = 10, $offset = 0) {
        ...
        $sql = "SELECT post_id FROM $db_post.posts WHERE group_id = $group_id ORDER BY is_pinned DESC, post_time DESC LIMIT $limit OFFSET $offset";
        $query = $this->db->query($sql);
        $posts = [];
        if ($query) {
